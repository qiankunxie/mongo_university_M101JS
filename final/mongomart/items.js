...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if (category !== 'All') {
            aggregationArray.push({$match: {category: category}});
        }
        aggregationArray.push({$skip: page * itemsPerPage});
        aggregationArray.push({$limit: itemsPerPage});
        database.collection('item').aggregate(aggregationArray, function (error, result) {
            callback(result);
        })
        // Qiankun' code ends here
    }
    ...
    this.getNumItems = function(category, callback) {
        ...
        aggregationArray.push({$group: { _id : null,num: {$sum: 1}}});
        database.collection('item').aggregate(aggregationArray, function (error, result) {
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        // Qiankun's code starts here  
        database.collection('item').find({ $text: { $search: query}}).skip(page * itemsPerPage).limit(itemsPerPage).toArray(function (err, res) {
            callback(res);
        })
        // Qiankun' code ends here
    }
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        database.collection('item').find({ $text: { $search: query}}).toArray(function (err, res) {
            callback(res.length);
        })
    }
    ...
    this.getItem = function(itemId, callback) {
        ...
        // to the callback.
        // Qiankun's code starts here  
        database.collection('item').find({ _id: itemId}).toArray(function (err, res) {
            callback(res[0]);
        })
        // Qiankun' code ends here
    }
